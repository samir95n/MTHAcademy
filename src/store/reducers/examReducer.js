...
const deleteFolder = (state, action) => {
  return {
    ...state,
    folders: state.folders.filter((folder) => folder.id !== action.deletedFolderId),
  };
};
...
function examReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case SET_TOTAL_SIZE:
      return setTotalSize(state, action);
    case SET_FILES:
      return setFiles(state, action);
    case SET_FOLDERS:
      return setFolders(state, action);
    case ADD_FOLDER:
    ...
    case STAR_FILE:
      return setStarFile(state, action);
    case STAR_FOLDER:
      return setStarFolder(state, action);
    default:
      return state;
  }
}
