...
import { getStudents, deleteUser, getAnswers } from '../../../../store/actions/adminActions';
...
function StudentsList(props) {
  ...
  const tHead = React.useMemo(() => {
    ...
    return head.filter((item) => !(props.role !== 'admin' && item.name === 'teacher'));
  }, []);
  const tBody = React.useMemo(() => {
    const body = props.students?.map((item, index) => {
      return [
        ...
        <span className="answersIcon" onClick={() => props.deleteUserHandle(item.id, 'students')}>
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    getStudents: (page, rowsPerPage) => dispatch(getStudents(page, rowsPerPage)),
